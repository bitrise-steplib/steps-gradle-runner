...
| `BITRISE_ANDROID_ARTIFACT_MAP_PATH` | Path of a JSON file, written next to the exported artifacts in the deploy directory, that records which exported APK/AAB files and which R8/ProGuard mapping file belong to the same build variant. Use it when a single build produces multiple variants and a later step (for example Google Play deploy) needs the matching mapping file for each app artifact.  File references inside the document are file names relative to the JSON file's own directory. Entries are keyed by Gradle module, then by variant — build variants are a per-module concept, and two modules can declare the same variant name. Example:  ``` {   "version": 1,   "modules": {     "app": {       "demoRelease": {         "mapping": "mapping.txt",         "aab": ["app-demo-release.aab"],         "apk": ["app-demo-release.apk"],         "aar": []       },       "paidRelease": {         "mapping": "mapping20260805121530.txt",         "aab": [],         "apk": ["app-paid-release.apk"],         "aar": []       }     }   },   "unmatched": { "apk": [], "aab": [], "aar": [], "mapping": [] },   "sources": {     "mapping.txt": "/bitrise/src/app/build/outputs/mapping/demoRelease/mapping.txt"   } } ```  `unmatched` lists exported files whose module/variant could not be derived from their build output path (for example artifacts written to non-standard locations). |
</details>
