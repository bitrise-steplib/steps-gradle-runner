...
| --- | --- |
| `BITRISE_APK_PATH` | This output will include the path of the generated APK file, after filtering based on the filter inputs. If the build generates more than one APK file which fulfills the filter inputs this output will contain the last one's path.  When building multiple variants, prefer `$BITRISE_ANDROID_ARTIFACT_MAP_PATH`: it tells you which APK belongs to which variant instead of only the last one. |
| `BITRISE_AAB_PATH` | This output will include the path of the generated AAB file, after filtering based on the filter inputs. If the build generates more than one AAB file which fulfills the filter inputs this output will contain the last one's path.  When building multiple variants, prefer `$BITRISE_ANDROID_ARTIFACT_MAP_PATH`: it tells you which AAB belongs to which variant instead of only the last one. |
...
| `BITRISE_APK_PATH_LIST` | This output will include the paths of the generated APK files, after filtering based on the filter inputs. The paths are separated with `\|` character, eg: `app-armeabi-v7a-debug.apk\|app-mips-debug.apk\|app-x86-debug.apk`  When building multiple variants, prefer `$BITRISE_ANDROID_ARTIFACT_MAP_PATH`: it groups the APKs by variant and pairs each variant with its mapping file. |
| `BITRISE_AAB_PATH_LIST` | This output will include the paths of the generated AAB files, after filtering based on the filter inputs. The paths are separated with `\|` character, eg: `app.aab\|app2.aab`  When building multiple variants, prefer `$BITRISE_ANDROID_ARTIFACT_MAP_PATH`: it groups the AABs by variant and pairs each variant with its mapping file. |
| `BITRISE_MAPPING_PATH` | **Deprecated**: when the build produces more than one mapping file this output is ambiguous — it only contains the *last* copied one, with no way to tell which APK/AAB it belongs to. Use `$BITRISE_ANDROID_ARTIFACT_MAP_PATH` instead, which pairs every variant's app files with that variant's mapping file. The output keeps working for backwards compatibility. |
| `BITRISE_ANDROID_ARTIFACT_MAP_PATH` | Path of a JSON file, written next to the exported artifacts in the deploy directory, that records which exported APK/AAB files and which R8/ProGuard mapping file belong to the same build variant. Use it when a single build produces multiple variants and a later step (for example Google Play deploy) needs the matching mapping file for each app artifact.  File references inside the document are file names relative to the JSON file's own directory. Entries are keyed by Gradle module, then by variant — build variants are a per-module concept, and two modules can declare the same variant name. Example:  ``` {   "version": 1,   "modules": {     "app": {       "demoRelease": {         "mapping": "mapping.txt",         "aab": ["app-demo-release.aab"],         "apk": ["app-demo-release.apk"]       },       "paidRelease": {         "mapping": "mapping20260805121530.txt",         "aab": [],         "apk": ["app-paid-release.apk"]       }     },     "mylibrary": {       "freeRelease": {         "mapping": "mapping20260805121531.txt",         "aab": [],         "apk": [],         "aar": ["mylibrary-free-release.aar"]       }     }   },   "unmatched": { "apk": [], "aab": [], "aar": [], "mapping": [] },   "sources": {     "mapping.txt": "/bitrise/src/app/build/outputs/mapping/demoRelease/mapping.txt"   } } ```  Library modules share the same tree: their build variant comes from the AAR's file name (AGP writes every variant into one `outputs/aar/` directory), and a library that minifies itself pairs with its own `mapping.txt`. The `aar` list is omitted for application modules.  `unmatched` lists exported files whose module/variant could not be derived from their build output path (for example artifacts written to non-standard locations, or an archive whose `archivesName` was customised). |
</details>
